...
def fix_reaction_activity_id(apps, schema_editor):
    with connection.cursor() as cursor:
        ...
        cursor.execute("SELECT COUNT(*) FROM eveuniverse_eveindustryactivity WHERE id=9 AND LOWER(name)='reactions'")
        if cursor.fetchone()[0]:
            # Vérifie qu'il n'y a pas déjà un id=11
            cursor.execute("SELECT COUNT(*) FROM eveuniverse_eveindustryactivity WHERE id=11")
            if not cursor.fetchone()[0]:
                # Met à jour l'id de 9 à 11
                cursor.execute("UPDATE eveuniverse_eveindustryactivity SET id=11 WHERE id=9 AND LOWER(name)='reactions'")
