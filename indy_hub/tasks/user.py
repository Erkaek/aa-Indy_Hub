...
# Indy Hub
from ..models import CharacterUpdateTracker, Blueprint, IndustryJob
...
def cleanup_inactive_user_data():
    ...
    for tracker in inactive_trackers:
        ...
        if blueprint_count > 0:
            count += 1
            logger.info(f"Cleaned up {blueprint_count} old blueprints for inactive user {user.username}")
...
def update_user_preferences_defaults():
    ...
    users_without_tracker = User.objects.filter(
        characterupdatetracker__isnull=True
    )
...
def sync_user_character_names():
    ...
    bp_char_ids = set(Blueprint.objects.exclude(
        character_id__isnull=True
    ).values_list('character_id', flat=True))
    
    job_char_ids = set(IndustryJob.objects.exclude(
        character_id__isnull=True
    ).values_list('character_id', flat=True))
    ...
    if all_char_ids:
        ...
        for bp in Blueprint.objects.filter(character_name="", character_id__in=all_char_ids):
            bp.refresh_from_db()
        
        # Update jobs with empty character names  
        for job in IndustryJob.objects.filter(character_name="", character_id__in=all_char_ids):
            job.refresh_from_db()
...
def generate_user_activity_report():
    ...
    users_with_blueprints = User.objects.filter(
        blueprint__isnull=False
    ).distinct().count()
    
    users_with_jobs = User.objects.filter(
        industryjob__isnull=False
    ).distinct().count()
