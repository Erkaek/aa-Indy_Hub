...
def sync_material_exchange_stock():
    ...
    try:
        ...
        with transaction.atomic():
            ...
            desired_ids = set(int(tid) for tid in stock_updates.keys())
            now = timezone.now()
            ...
            if to_delete:
                ...
                logger.info("Deleted %d obsolete stock items for config %s", deleted_count, config.pk)
            ...
            if not desired_ids and current_ids:
                ...
                logger.info("Cleared all stock items for config %s (no assets in structure)", config.pk)
            ...
            if to_create:
                ...
                logger.info("Created %d new stock items for config %s", len(to_create), config.pk)
            ...
            if to_update:
                ...
                logger.info("Updated %d stock items for config %s", len(to_update), config.pk)
...
def sync_material_exchange_prices():
    ...
    try:
        ...
        logger.info(f"Material Exchange prices sync completed: {len(type_ids)} types updated")
...
def verify_pending_sell_payments():
    ...
    try:
        config = MaterialExchangeConfig.objects.first()
        if not config:
            logger.warning("Material Exchange not configured - skipping payment verification")
            return
        ...
        for order in pending_orders:
            ...
            seller_id = order.seller.profile.main_character.character_id if hasattr(order.seller, 'profile') else None
            ...
            for entry in journal_data:
                ...
                amount_matches = abs(entry_amount - expected_amount) < (expected_amount * 0.01)
        ...
        if verified_count > 0:
            logger.info(f"Material Exchange payment verification: {verified_count} orders verified")
