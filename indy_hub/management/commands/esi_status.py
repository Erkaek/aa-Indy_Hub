...
from indy_hub.utils import get_esi_status, clear_esi_cache
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if options['clear_cache']:
            clear_esi_cache()
            self.stdout.write(
                self.style.SUCCESS('ESI cache cleared successfully')
            )
            return
        ...
        if options['reset_circuit_breakers']:
            ...
            self.stdout.write(
                self.style.SUCCESS('Circuit breakers reset successfully')
            )
            return
