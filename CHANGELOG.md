...
- Administration: added `rebuild_admin_user_statuses` and `rebuild_indy_hub_usage_rollups` management commands for operational reconciliation and backfills.
...
- Settings / User admin now relies on indexed precomputed account state, normalized usage rollups, incremental updates, and bounded Celery reconciliation/backfill so listings remain database-paginated and never refresh OAuth/ESI tokens during passive reads.
- Migrations: added `0111_add_user_favorite_structure`, `0112_indyhubuserusage`, the indexed precomputed user-administration state, and normalized daily usage rollups; large historical populations are rebuilt asynchronously.
- Tests: added/extended focused coverage for favorites toggles, user-admin visibility, global search/filter/sort/pagination behavior, passive token handling, precomputed-state reconstruction, usage rollups and query shape, craft payload API behavior, and industry chat services.
...
- Packaging: removed Alliance Auth, AppUtils, django-esi, and django-eveonline-sde from Indy Hub's dependency set so installing or reinstalling Indy Hub cannot upgrade an existing Alliance Auth v4 instance to v5 through transitive dependency resolution. The previously documented `aa4` / `aa5` extras have been removed for the same reason — install Alliance Auth with its own command, then install Indy Hub on top.
...
- Crafting Projects / Financial planner: fixed the *Total Material Cost*, *Cash Needed for Materials*, *Cash Investment Needed*, *Total Cost* and top-of-page summary cards reading 0 ISK when every project item was switched to *Buy*. The financial-row aggregator silently dropped any item whose `type_id` matched a project final-output, so the buy cost of items the project intends to resell was never counted. The filter now respects the user's per-item Buy/Prod choice: items toggled to *Buy* are listed both as a buy line (cost) and as a revenue line (sell price), so per-line totals and the summary row now agree, and the summary cards no longer disagree with the table (issue #71).

- Crafting Projects / Decisions: clicking *Set everything to Buy* / *Set everything to Prod* on the **Tree** tab now works even when the **Decisions** tab has not been opened yet. Previously the helper looked exclusively at `#decisionStrategyRows`, which is rendered lazily, and bailed out early when that container was empty so no switches were updated. The lookup now falls back to the materials tree when the Decisions container has not been hydrated (issue #71).

- Crafting Projects / Financial planner: fixed *Total Revenue* being inflated by phantom "surplus revenue" when every project item is in *Buy* mode. With no items in *Prod*, `SimulationAPI.getProductionCycles()` correctly returned an empty array, but the `recalcFinancials` fallback then re-read the static `craft_cycles_summary` payload (computed server-side as if everything was being produced) and credited those rounding surpluses to revenue. The fallback now only triggers for legacy payloads that do not expose `getProductionCycles`; when the live API is available, an empty cycles array means zero surplus revenue, so cost and revenue agree line-for-line in all-buy mode (issue #71).

- Crafting Projects / Decisions: fixed saved Buy/Prod decisions silently reverting to the default "everything in *Prod*" view after reloading the project page. The frontend persisted the user's choices in `workspace_state.buyTypeIds` and on the cached `materials_tree` (`project_inclusion_mode='buy'`), but the SimulationAPI hard-coded every craftable's initial switch state to `'prod'` on ingest, ignoring the inclusion mode carried by the tree nodes. Worse, the DOM restoration step ran before the Decisions tab was lazily rendered, so it had no `.mat-switch` element to flip. The SimulationAPI now seeds switch states from the saved `project_inclusion_mode`, so opening the Decisions tab after a reload shows the previously saved Buy/Prod choices instead of resetting to defaults (issue #71).

- Crafting Projects / Stock allocations: prevented saved stock allocations from being silently destroyed on the next save. `collectCraftPageSessionState` was persisting the *current-plan-clamped* view of allocations (clamped to `MIN(requiredQty, availableQty, requestedQty)`), so any allocation whose underlying intermediate was momentarily not required by the current plan — e.g. a stale character stock snapshot reporting `availableQty=0`, or a buy/prod toggle that switched the intermediate to produce, or a runs reduction — was zeroed out and dropped from the DB on save. The persistence path now stores the raw user intent (only normalized to non-negative integers); display-time clamping still happens via `getCraftStockAllocationSummary`. Additionally, `getCraftNormalizedStockAllocationsForCurrentPlan` no longer clobbers `craftBPFlags.stockAllocations` with its clamped view as a side effect, so simply opening the Stock or Financial Planner tab can no longer wipe out saved allocations that are not currently required (issue #71).
