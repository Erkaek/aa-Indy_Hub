...
- Crafting Projects: reaction formulas are no longer rendered at all in the project *Blueprints* tab. Reaction blueprints cannot have non-zero ME/TE in EVE, so the per-blueprint card with ME/TE inputs and copy controls was misleading. Reactions are still surfaced in the *Cycles* and *Materials* tabs so they remain accounted for in the production plan (issue #69 follow-up).
